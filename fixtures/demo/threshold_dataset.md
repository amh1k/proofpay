...
Policy `proofpay-policy-1.0.0`, fingerprint `1b269461a49561ff`. Every number below
...
| `missing_evidence_penalty` | `0.25` | Weight an unreadable field keeps in the aggregate's denominator (0.25 = quarter-weight absence). |
...
The engine reaches the manifest's **status on 18 of 30** cases,
and its status *and* stated reason on **10 of 30**. Those gaps are
...
| `N02` | NEEDS_REVIEW | NEEDS_REVIEW / AMOUNT_OVERPAID | VERIFIED | R080 | AMOUNT_OVERPAID, CLAIM_CONSISTENT, STRONG_FIELD_AGREEMENT | **no** |
...
| `N06` | NEEDS_REVIEW | NEEDS_REVIEW / LOW_EXTRACTION_CONFIDENCE | VERIFIED | R090 | AMOUNT_EXACT, CLAIM_CONSISTENT, STRONG_FIELD_AGREEMENT | **no** |
...
| `N03` | 0.512 | 0.7245 | -0.0955 | 0.0000 | 2 | no | amount=AMT_EXACT, reference=REF_MISSING, sender_name=NAME_COMMON_ONLY, timestamp=TS_TIGHT |
...
| `N05` | 0.829 | 0.7571 | -0.0629 | lone | 1 | yes | amount=AMT_EXACT, reference=REF_EXACT, sender_name=NAME_COMMON_ONLY, timestamp=TS_MISSING |
| `N06` | 0.870 | 0.8407 | +0.0207 | lone | 1 | yes | amount=AMT_EXACT, reference=REF_EXACT, sender_name=NAME_MISSING, timestamp=TS_DATE_ONLY |
...
| `tau_accept` | `0.82` | `0.76` | `0.85` | `G03`, `G04`, `G05`, `G06`, `G08`, `G09`, `G10`, `N04`, `N06` | `N01` |
| `tau_reject` | `0.45` | none | `0.57` | `S02`, `S03`, `S04`, `S05`, `S06` | `N03` |
| `tau_margin` | `0.1` | none | none | -- | `N03` |
| `tau_ambiguous` | `0.45` | none | none | -- | `N03` |
| `missing_evidence_penalty` | `0.25` | none | `0.38` | `N06` | -- |
| `w_reference` | `1` | `0.6` | `1.9` | `N06`, `S02` | `N01` |
| `w_amount` | `1` | `0.6` | `1.9` | `N06`, `S03`, `S04`, `S05`, `S06` | `N01` |
| `w_timestamp` | `0.8` | `0` | `1.1` | `D02`, `D03`, `G01`, `N02`, `N06`, `S02`, `S03`, `S04`, `S05`, `S06` | `N01` |
| `w_sender_name` | `0.6` | `0.3` | `0.9` | `D02`, `D03`, `G01`, `N02`, `N05`, `N06` | `N01` |
...
- lower it to `0.76`: `G03` NEEDS_REVIEW -> VERIFIED; `G04` NEEDS_REVIEW -> VERIFIED; `G05` NEEDS_REVIEW -> VERIFIED; `G06` NEEDS_REVIEW -> VERIFIED; `G08` NEEDS_REVIEW -> VERIFIED; `G09` NEEDS_REVIEW -> VERIFIED; `G10` NEEDS_REVIEW -> VERIFIED; `N04` NEEDS_REVIEW -> VERIFIED
- raise it to `0.85`: `N06` VERIFIED -> NEEDS_REVIEW
...
**`missing_evidence_penalty`** (default `0.25`)

- raise it to `0.38`: `N06` VERIFIED -> NEEDS_REVIEW

**`w_reference`** (default `1`)

- lower it to `0.6`: `N06` VERIFIED -> NEEDS_REVIEW
- raise it to `1.9`: `S02` NEEDS_REVIEW -> UNMATCHED

**`w_amount`** (default `1`)

- lower it to `0.6`: `N06` VERIFIED -> NEEDS_REVIEW
- raise it to `1.9`: `S03` SUSPICIOUS -> UNMATCHED; `S04` SUSPICIOUS -> UNMATCHED; `S05` SUSPICIOUS -> UNMATCHED; `S06` SUSPICIOUS -> UNMATCHED

**`w_timestamp`** (default `0.8`)

- lower it to `0`: `G01` VERIFIED -> NEEDS_REVIEW; `S02` NEEDS_REVIEW -> UNMATCHED; `S03` SUSPICIOUS -> UNMATCHED; `S04` SUSPICIOUS -> UNMATCHED; `S05` SUSPICIOUS -> UNMATCHED; `S06` SUSPICIOUS -> UNMATCHED; `D02` VERIFIED -> NEEDS_REVIEW; `D03` VERIFIED -> NEEDS_REVIEW; `N02` VERIFIED -> NEEDS_REVIEW
- raise it to `1.1`: `N06` VERIFIED -> NEEDS_REVIEW

**`w_sender_name`** (default `0.6`)

- lower it to `0.3`: `N05` NEEDS_REVIEW -> VERIFIED
- raise it to `0.9`: `G01` VERIFIED -> NEEDS_REVIEW; `D02` VERIFIED -> NEEDS_REVIEW; `D03` VERIFIED -> NEEDS_REVIEW; `N02` VERIFIED -> NEEDS_REVIEW; `N06` VERIFIED -> NEEDS_REVIEW
...
- raise it to `0.73`: `N03` R050/[AMBIGUOUS_CANDIDATES, AMOUNT_EXACT, CLAIM_CONSISTENT] -> R999/[AMOUNT_EXACT, CLAIM_CONSISTENT]
...
constrains all 30 thresholds equally will waste an afternoon.

Of 30 thresholds, **11 can change a verdict**,
**3 change only the explanation**, and **16 change nothing
...
| `tau_accept` | `0.76` | `0.85` | 9 (`G03`, `G04`, `G05`, `G06`, `G08`, `G09`, `G10`, `N04`, `N06`) |
| `tau_reject` | none | `0.57` | 5 (`S02`, `S03`, `S04`, `S05`, `S06`) |
| `missing_evidence_penalty` | none | `0.38` | 1 (`N06`) |
| `w_reference` | `0.6` | `1.9` | 2 (`N06`, `S02`) |
| `w_amount` | `0.6` | `1.9` | 5 (`N06`, `S03`, `S04`, `S05`, `S06`) |
| `w_timestamp` | `0` | `1.1` | 10 (`D02`, `D03`, `G01`, `N02`, `N06`, `S02`, `S03`, `S04`, `S05`, `S06`) |
| `w_sender_name` | `0.3` | `0.9` | 6 (`D02`, `D03`, `G01`, `N02`, `N05`, `N06`) |
...
- **7 of the disagreements in section 2 are unreachable by any threshold.** These cases expect a reason code that no rule in `rules_v1.py`
  carries (`LOW_EXTRACTION_CONFIDENCE`, `PROOF_REUSED`, `REFERENCE_MISMATCH`, `TIMESTAMP_MISMATCH`):
  `U03`, `S02`, `D02`, `D03`, `N04`, `N05`, `N06`. No policy value causes the engine to
