...
def scoped_select[T: Base](model: type[T], merchant_id: uuid.UUID) -> Select[tuple[T]]:
    """Start every repository query with the tenant predicate."""
    return select(model).where(model.merchant_id == merchant_id)


def one_or_none[T: Base](session: Session, statement: Select[tuple[T]]) -> T | None:
    """Execute a scoped single-row query with a typed result."""
    return session.scalars(statement).one_or_none()


__all__ = ["as_uuid", "one_or_none", "scoped_select"]
